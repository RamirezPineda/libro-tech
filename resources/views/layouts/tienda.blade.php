...
<div x-data="{ cartOpen: false , isOpen: false }" class="bg-white dark:bg-gray-800">
        ...
        <div class="container mx-auto px-6 py-3">
            <div class="flex items-center justify-between">
                ...
                <div class="flex items-center justify-end w-full">
                    <button @click="cartOpen = !cartOpen" class="text-gray-600 dark:text-gray-300 focus:outline-none mx-4 sm:mx-0">
                    ...
                    <form action="{{route('tienda.setTheme')}}" method="POST">
                      @csrf
                        @if ($modo == 'dark')
                          <button type="submit" class="text-gray-600 dark:text-gray-200 focus:outline-none mx-4 sm:mx-3 py-1">
                    ...
                    @if (Auth::user())
                    <a href="{{ route('dashboard') }}" target="_blank" class="mx-4 sm:mx-3  text-sm px-2 py-2 bg-blue-600 text-white  font-medium rounded hover:bg-blue-500 focus:outline-none focus:bg-blue-500">
                    Dashboard
                    </a>
                    @else
                      <a href="{{ route('login') }}" target="_blank" class="mx-4 sm:mx-3 text-sm px-2 py-2 bg-blue-600 text-white  font-medium rounded hover:bg-blue-500 focus:outline-none focus:bg-blue-500">
                        Iniciar Sesión
                      </a>
                    @endif
                    ...
                    <div class="flex sm:hidden">
                        <button @click="isOpen = !isOpen" type="button" class="text-gray-600 dark:text-gray-300 hover:text-gray-500 focus:outline-none focus:text-gray-500" aria-label="toggle menu">
            ...
            <nav :class="isOpen ? '' : 'hidden'" class="sm:flex sm:justify-center sm:items-center mt-4">
                <div class="flex flex-col sm:flex-row">
                    <a class="mt-3 text-gray-600 dark:text-gray-300 hover:underline sm:mx-3 sm:mt-0" href="{{ route('tienda.index') }}">Tienda</a>
                    <a class="mt-3 text-gray-600 dark:text-gray-300 hover:underline sm:mx-3 sm:mt-0" href="{{ route('carrito') }}">Carrito de Compras</a>
                </div>
            </nav>
    ...
    {{-- CARRITO DE COMPRAS --}}
    <div  :class="cartOpen ? 'translate-x-0 ease-out' : 'translate-x-full ease-in'" class="fixed right-0 top-0 max-w-xs w-full h-full px-6 py-4 transition duration-300 transform overflow-y-auto bg-white dark:bg-gray-800 border-l-2 border-gray-300 dark:border-gray-600">
        <div class="flex items-center justify-between">
    ...
    <footer class="bg-gray-200 dark:bg-gray-900">
        <div class="container mx-auto px-6 py-3 flex justify-between items-center">
            <a href="#" class="text-xl font-bold text-gray-500 dark:text-gray-200 hover:text-gray-400">Book Mind</a>
  ...
  cart.forEach(product => {
    ...
    productElement.innerHTML = `
      <div class="flex">
        ...
        <div class="mx-3">
          <h3 class="text-sm text-gray-600 dark:text-gray-200">${product.titulo}</h3>
          <div class="flex items-center mt-2">
...
<script>
  ...
  const pagarForm = () => {
    ...
    if (cartForm && totalForm) {
      ...
      const { subTotal, descuentoTotal } = cart.reduce((result, product) => {
  ...
  const actualizarListaDeCarrito = () => {
    ...
    cart.forEach(product => {
      ...
      listItem.innerHTML = `
        <img src="${product.imagen}" alt="${product.titulo}" class="h-32 w-32 rounded object-cover">

        <div>
          <h3 class="text-gray-700 dark:text-gray-300">${product.titulo}</h3>
...
const addToCart = (productoId, productoTitulo, productoAutor, productoPrecio, productoImagen, productoDescuento) => {
  let cart = JSON.parse(localStorage.getItem('cart')) || [];
